...
SELECT DISTINCT
    TO_CHAR(PATDEM.PATID) AS PNT_IDENTIFIER, 
...
FROM 
    DEMOGRAPHIC PATDEM
    LEFT JOIN ACT_FHIR_MAPPING TCC1 ON TCC1.COLUMN_CD='SEX' AND PATDEM.SEX_CD=TCC1.LOCAL_IN_CD AND TCC1.TABLE_CD ='DEMOGRAPHIC'
    LEFT JOIN ACT_FHIR_MAPPING TCC3 ON TCC3.COLUMN_CD='RACE' AND PATDEM.RACE_CD=TCC3.LOCAL_IN_CD AND TCC1.TABLE_CD ='DEMOGRAPHIC'
    LEFT JOIN ACT_FHIR_MAPPING TCC4 ON TCC4.COLUMN_CD='HISPANIC' AND PATDEM.ETHNICITY_CD=TCC4.LOCAL_IN_CD AND TCC4.TABLE_CD ='DEMOGRAPHIC'
...
SELECT DISTINCT
        VISIT.ENCOUNTERID AS ENC_IDENTIFIER,
        'unknown' AS ENC_STATUS,
        'Patient/'||VISIT.PATID AS ENC_SUBJECT_REFERENCE,
	    CASE 
	        WHEN VISIT.PROVIDERID IS NULL THEN NULL ELSE 'Practitioner/'||VISIT.PROVIDERID 
        ...
        VISIT.ADMIN_DATE AS ENC_PERIOD_START,
        VISIT.DISCHARGE_DATE AS ENC_PERIOD_END,
...
FROM 
	(ENCOUNTER VISIT
        LEFT JOIN ACT_FHIR_MAPPING TCC1 ON TCC1.COLUMN_CD='ENC_TYPE' AND VISIT.ENC_TYPE=TCC1.LOCAL_IN_CD) AND TCC1.TABLE_CD ='ENCOUNTER'
...
SELECT DISTINCT
	DX.PATID || '~'
    || CASE WHEN DX.ENCOUNTERID IS NULL THEN '' ELSE DX.ENCOUNTER_NUM || '~') END
    || DX.DX_TYPE || '~'
    || DX.DX || '~'
    || TO_CHAR(DX.DX_DATE,'YYYYMMDD')|| '000000'
    AS CON_IDENTIFIER, 
...
SELECT DISTINCT
    ...
    || PX.PX_TYPE || '~'
    || PX.PX || '~'
    || TO_CHAR(PX.PX_DATE,'YYYYMMDD')|| '000000.0' || '~'
    || PX.PROVIDERID AS PCD_IDENTIFIER,
	...
	CASE
	    ...
	    WNEH PX.PX_TYPE = 'CH' AND PX.PX != '99999' THEN 'http://www.ama-assn.org/go/cpt/'
...
SELECT DISTINCT
    LABS.ENCOUNTERID || '~' || LABS.OBSCLIN_TYPE || '~' || LABS.OBSCLIN_CODE || '~' || TO_CHAR(LABS.OBSCLIN_DATE,'YYYYMMDD')|| '000000.1' || '~' ||  LABS.PROVIDERID   AS OBS_IDENTIFIER, 
    'final' AS OBS_STATUS,
    ...
    LABS.OBSCLIN_CODE AS OBS_CODE_CODING_CODE,
    NULL AS OBS_CODE_CODING_DISPLAY,
    LABS.OBSCLIN_RESULT_NUM AS OBS_VALUEQUANTITY_VALUE,
    CASE
        WHEN LABS.OBSCLIN_RESULT_MODIFIER IN ('EQ', 'GE','GT','LE','LT') THEN TCC1.FHIR_OUT_CD
        ELSE NULL
    END AS OBS_VALUEQUANTITY_COMPARATOR,
    LABS.OBSCLIN_RESULT_UNIT AS OBS_VALUEQUANTITY_CODE,
    LABS.OBSCLIN_RESULT_TEXT AS OBS_VALUESTRING,
    LABS.OBSCLIN_DATE AS OBS_ISSUED, 
    NULL AS OBS_EFFECTIVEDATETIME,
    NULL AS OBS_REFRANGE_LOW,
    NULL AS OBS_REFRANGE_HIGH,
--    TCC2.FHIR_OUT_CD AS OBS_INTERPRETATION_CODE, 
    'http://hl7.org/fhir/ValueSet/observation-interpretation' AS OBS_INTERPRETATION_SYST 
FROM
    OBS_GEN LABS
    LEFT JOIN ACT_FHIR_MAPPING TCC1 ON TCC1.COLUMN_CD='OBSCLIN_RESULT_MODIFIER' AND LABS.OBSCLIN_RESULT_MODIFIER=TCC1.LOCAL_IN_CD AND TCC1.TABLE_CD ='OBS_CLIN'
--    LEFT JOIN ACT_FHIR_MAPPING TCC2 ON TCC2.COLUMN_CD='VALUEFLAG_CD' AND LABS.VALUEFLAG_CD=TCC2.LOCAL_IN_CD AND TCC1.TABLE_CD ='OBS_CLIN'

UNION

SELECT DISTINCT
    LABS.ENCOUNTERID || '~' || LABS.OBSGEN_TYPE || '~' || LABS.OBSGEN_CODE || '~' || TO_CHAR(LABS.OBSGEN_DATE,'YYYYMMDD')|| '000000.1' || '~' ||  LABS.PROVIDERID   AS OBS_IDENTIFIER, 
    'final' AS OBS_STATUS,
    'Patient/'||LABS.PATID AS OBS_SUBJECT_REFERENCE, 
    'Encounter/'||LABS.ENCOUNTERID AS OBS_CONTEXT_REFERENCE,
    'http://hl7.org/fhir/ValueSet/observation-category' AS OBS_CATEGORY_SYST,
    'laboratory' AS OBS_CATEGORY_CODE,
    'Laboratory' AS OBS_CATEGORY_DISPLAY,
    LABS.OBSGEN_TYPE AS OBS_CODE_CODING_SYST,
    LABS.OBSGEN_CODE AS OBS_CODE_CODING_CODE,
    NULL AS OBS_CODE_CODING_DISPLAY,
    LABS.OBSGEN_RESULT_NUM AS OBS_VALUEQUANTITY_VALUE,
    CASE
        WHEN LABS.OBSGEN_RESULT_MODIFIER IN ('EQ', 'GE','GT','LE','LT') THEN TCC1.FHIR_OUT_CD
        ELSE NULL
    END AS OBS_VALUEQUANTITY_COMPARATOR,
    LABS.OBSGEN_RESULT_UNIT AS OBS_VALUEQUANTITY_CODE,
    LABS.OBSGEN_RESULT_TEXT AS OBS_VALUESTRING,
    LABS.OBSGEN_DATE AS OBS_ISSUED, 
    NULL AS OBS_EFFECTIVEDATETIME,
    NULL AS OBS_REFRANGE_LOW,
    NULL AS OBS_REFRANGE_HIGH,
--    TCC2.FHIR_OUT_CD AS OBS_INTERPRETATION_CODE, 
...
FROM
    OBS_GEN LABS
    LEFT JOIN ACT_FHIR_MAPPING TCC1 ON TCC1.COLUMN_CD='OBSCLIN_RESULT_MODIFIER' AND LABS.OBSGEN_RESULT_MODIFIER=TCC1.LOCAL_IN_CD AND TCC1.TABLE_CD ='OBS_CLIN'
--    LEFT JOIN ACT_FHIR_MAPPING TCC2 ON TCC2.COLUMN_CD='VALUEFLAG_CD' AND LABS.VALUEFLAG_CD=TCC2.LOCAL_IN_CD AND TCC1.TABLE_CD ='OBS_GEN'
);
